...
Future<AudioHandler> initAudioService() async {
  return await AudioService.init(
      ...
      config: AudioServiceConfig(
        androidNotificationChannelId: 'com.vvpswami.bg_demo.channel.audio',
        androidNotificationChannelName: 'VVP Swami Audio playback',
...
class MyAudioHandler extends BaseAudioHandler implements AudioPlayerHandler {
  ...
  Future<bool> isAudioLocal(MediaItem mediaItem) async {
    String extension = path.extension(mediaItem.extras!['url'] as String);
    print(mediaItem.extras);
    String title = 'file${mediaItem.id}$extension';
  ...
  Future<String> getLocalFilePath(MediaItem mediaItem) async {
    String extension = path.extension(mediaItem.extras!['url'] as String);
    print(mediaItem.extras);
    String title = 'file${mediaItem.id}$extension';
    final directory = await getApplicationDocumentsDirectory();
  ...
  Future<List<UriAudioSource>> createAudioSources(
      List<MediaItem> mediaItems) async {
    List<UriAudioSource> newList = [];

    for (var item in mediaItems) {
      bool isLocal = await isAudioLocal(item);

      UriAudioSource itemSource = !isLocal
          ? AudioSource.uri(Uri.parse(item.extras!['url'] as String), tag: item)
          : AudioSource.file(await getLocalFilePath(item), tag: item);
